  ...
  .post(
    celebrate({
      [Segments.BODY]: Joi.object().keys({
        ...
        primeiroNome: Joi.string().allow('').max(500),
        sobrenome: Joi.string().allow('').max(500),
        telefone: Joi.string().allow(null, '').max(500),
        nomeOrganizador: Joi.string().allow('').max(500),
        cnpj: Joi.string().allow('').max(500),
        nomeAdmin: Joi.string().allow('').max(500),
